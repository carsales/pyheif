...
def write(raw, fmt, quality=50, lossless=False):
    ...
    image = p_image[0]
    error = _libheif_cffi.lib.heif_image_create(width, height, colorspace, chroma, image)
    error = _libheif_cffi.lib.heif_image_add_plane(image, _constants.heif_channel_interleaved, width, height, depth)
    encoder_descriptors = _libheif_cffi.ffi.new("const heif_encoder_descriptor *")
    _libheif_cffi.lib.heif_context_get_encoder_descriptors(context, compression, _libheif_cffi.ffi.NULL, encoder_descriptors, 5)
    
    p_encoder = _libheif_cffi.ffi.new("heif_encoder *")
    encoder = p_encoder[0]
    error = _libheif_cffi.lib.heif_context_get_encoder(context, encoder_descriptors[0], encoder)
    ...
    handle = p_handle[0]
    error = _libheif_cffi.lib.heif_context_encode_image(context, image, encoder, _libheif_cffi.ffi.NULL, handle)
