...
def read_heif(fp, apply_transformations=True):
    ...
    return read(fp, apply_transformations)
...
def _read_heif_bytes(d, apply_transformations, convert_hdr_to_8bit):
    ...
    if filetype_check == constants.heif_filetype_no:
        raise ValueError("Input is not a HEIF/AVIF file")
    elif filetype_check == constants.heif_filetype_yes_unsupported:
...
def _read_heif_handle(handle, apply_transformations, convert_hdr_to_8bit):
    ...
    colorspace = constants.heif_colorspace_RGB
    if bit_depth == 8:
        if has_alpha:
            chroma = constants.heif_chroma_interleaved_RGBA
        else:
            chroma = constants.heif_chroma_interleaved_RGB
    else:
        if has_alpha:
            chroma = constants.heif_chroma_interleaved_RRGGBBAA_BE
        else:
            chroma = constants.heif_chroma_interleaved_RRGGBB_BE
    ...
    try:
        data, size, stride = _read_heif_image(img)
    finally:
...
def _read_color_profile(handle):
    ...
    if profile_type == constants.heif_color_profile_type_not_present:
        return

    color_profile = {"type": "unknown", "data": None}
    if profile_type == constants.heif_color_profile_type_nclx:
        color_profile["type"] = "nclx"
    elif profile_type == constants.heif_color_profile_type_rICC:
        color_profile["type"] = "rICC"
    elif profile_type == constants.heif_color_profile_type_prof:
...
def _read_heif_image(img):
    width = _libheif_cffi.lib.heif_image_get_width(
        img, constants.heif_channel_interleaved
    )
    height = _libheif_cffi.lib.heif_image_get_height(
        img, constants.heif_channel_interleaved
    )
    size = (width, height)
    ...
    p_data = _libheif_cffi.lib.heif_image_get_plane_readonly(
        img, constants.heif_channel_interleaved, p_stride
    )
    stride = p_stride[0]
